...
def psf_and_parameter_testing():
    ...
    input_name = r'D:\Janelia\Data 2024-06-30\488cm cell 5 good signal_20240627_131236  Fail vector tuning\488 nm 5 phases 0.81 NA React_All Linear SIM_cam1_1.mrc'
    ...
    for line_width in [0.0311]:#[20.,2.,0.2, 0.02]:#[2, 0.2, 0.02]:
        for angles in [3.54]:#[10, 1, 0.1]:
            
            ls = (wl/1000)/2/0.81
            ls = ls
            #ls = 0.2035)
            
            user_text = 'gApo_'+str(gammaApo)+'_supR_'+str(suppressR)+'_w_'+str(wiener)+'_wl_'+str(wl)+'_ls_'+f'{ls:.2f}'
            ...
            if nok0search:
                user_text += '_nok0search'

            print(user_text)
            
            base_kwargs.update(dict(apodizeoutput=apodizeoutput, gammaApo=gammaApo, suppressR=suppressR, wiener=wiener, nosuppress=nosuppress, driftfix = driftfix, noOrder0=noOrder0, zpadto=zpadto, makemodel=makemodel, nok0search=nok0search, forcemodamp=[1.0,1.0]))   # default Full frame Recon. parameters    
            sim_kwargs = dict(                                                                                                            

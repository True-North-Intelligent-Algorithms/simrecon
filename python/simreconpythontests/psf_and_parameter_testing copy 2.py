...
def psf_and_parameter_testing():
    ...
    input_name = r'D:\Janelia\test_data\roi128.mrc'
    ...
    for gammaApo in [0.03]:#[20.,2.,0.2, 0.02]:#[2, 0.2, 0.02]:
        for suppressR in [1.0]:#[10, 1, 0.1]:
            for wiener in [0.001]:#[0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]:

                input_base_name = os.path.basename(input_name)
                input_base_name = os.path.splitext(input_base_name)[0]

                user_text = input_base_name+'_gApo_'+str(gammaApo)+'_supR_'+str(suppressR)+'_w_'+str(wiener)+'_wl_'+str(wl)
                ...
                base_kwargs.update(dict(gammaApo=gammaApo, suppressR=suppressR, wiener=wiener, nosuppress=nosuppress, driftfix = driftfix, noOrder0=noOrder0, zpadto=zpadto, makemodel=makemodel))   # default Full frame Recon. parameters    
                sim_kwargs = dict(                                                                                                            
                ...
                fig = show_xyz_slice(reconstructed, reconstructed.shape[2]//2, reconstructed.shape[1]//2, reconstructed.shape[0]//2, sxy=1, sz=5)
                ...
                yc = 50#697
                xc = 50#843
                #reconstructed_crop = centercrop(reconstructed, (reconstructed.shape[0], yc, xc))
                ...
                fig = show_xyz_slice_center(fft_app_mag, sxy=1, sz=5, gamma=0.2, colormap='inferno')
                fig.savefig(os.path.join(figure_path, user_text + "_fft_center_mag.png"))

                fig = show_xyz_slice_center(fft_app_mag_log, sxy=1, sz=5)

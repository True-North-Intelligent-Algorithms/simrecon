...
def split_process_recombine(fullpath, tile_size, padding, sim_kwargs, bg_estimate=None, window_func=cosine_edge, tile_data = None, tile_limits = None):
    ...
    with tempfile.TemporaryDirectory(dir="D:/") as dir_name:
        ...
        if oldmrc.m._mmap.closed:
            print("MRC Raw data file is now closed")
        else: 
            print('MRC Raw data file is still open')

        if hasattr(temp_mrc,'m') and isinstance(temp_mrc.m, np.memmap):
            if temp_mrc.m._mmap.closed:
                print("MRC temp data file is now closed")
            else: 
                print('MRC temp data file is still open')
        else:
            print('problem with temp mrc object access')

        
        if hasattr(oldmrc,'m') and isinstance(oldmrc.m, np.memmap):
            oldmrc.m._mmap.close()
            if oldmrc.m._mmap.closed:
                print("MRC Raw data file is now closed")
            else: 
                print('MRC Raw data file is still open')
